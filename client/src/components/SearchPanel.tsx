...
const FilterLabel = ({ title, htmlFor }: { title: string, htmlFor: string }) => {
  return (
    <label
      ...
      // className="text-white sm:text-gray-600 sm:ml-4 sm:mr-4 font-semibold w-12 sm:w-16 inline-block sm:text-sm tv:text-lg mr-3"
      htmlFor={htmlFor}
    >
      {title}
    </label>
...
const SearchPanel = ({ statusFilter, typeFilter, addNewFilm, setStatusFilter, setSearch, setTypeFilter }: IProps) => {
  return (
    <div className="sm:bg-gray-200 sm:rounded-lg sm:shadow overflow-hidden py-0 sm:py-2">
      <div className="whitespace-nowrap sm:inline inline-block items-center w-full relative rounded-md shadow-sm grow">
        <ButtonGroup.Button
          className="sm:ml-2 px-4 py-2 sm:w-40 w-full border text-sm tv:text-lg font-medium text-white rounded-lg hover:bg-green-700 bg-green-800"
          onClick={addNewFilm}
        >
          Add new film
        </ButtonGroup.Button>
      </div>
      <div className="my-2 whitespace-nowrap lg:inline flex items-center w-full">
        <FilterLabel title="Search" htmlFor="search" />
        <div className="relative rounded-md shadow-sm inline-block grow sm:grow-0 w-full md:w-max sm:mr-4 md:mr-0">
